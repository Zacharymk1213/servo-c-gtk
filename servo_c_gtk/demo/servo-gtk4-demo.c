...
on_create_web_view(ServoGtkWebView *web_view, guint64 request_id, gpointer user_data)
{
    ...
    gtk_window_set_child(GTK_WINDOW(window), popup);
...
activate(GtkApplication *app, gpointer user_data)
{
    ...
    gtk_box_append(GTK_BOX(url_bar), zoom_out_button);

    zoom_reset_button = gtk_button_new_from_icon_name("zoom-original-symbolic");
    ...
    gtk_box_append(GTK_BOX(url_bar), zoom_reset_button);
    ...
    gtk_box_append(GTK_BOX(url_bar), zoom_in_button);

    dialog_button = gtk_button_new_from_icon_name("dialog-question-symbolic");
    ...
    gtk_box_append(GTK_BOX(url_bar), dialog_button);
